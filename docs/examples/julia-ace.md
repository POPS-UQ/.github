...
interatomic potential on the `Si_tiny` silicon dataset from
[Bartók et al., 2018](https://doi.org/10.1103/PhysRevX.8.041048), provided as an
artifact by [`ACEpotentials.jl`](https://github.com/ACEsuit/ACEpotentials.jl).
We then use POPS regression to attach predictive uncertainties to the energies,
forces and virials on a held-out test set.

The full script lives in the repository at
[`examples/ace/pops_ace.jl`](https://github.com/POPS-UQ/POPSRegression.jl/blob/main/examples/ace/pops_ace.jl);
see [Installation](../installation.md#example-environments) for how to
instantiate its environment and run it.

## Setting up an ACE linear regression problem

We start by following the `ACEpotentials` workflow, loading the dataset and
...
Next we define an ACE linear model and assemble the design matrices from the
ACE basis functions.
...
ACE linear models are usually fitted in a preconditioned basis. Here we use the
algebraic smoothness prior provided by `ACEpotentials.jl`.
...
We now fit the POPS hypercube ensemble. A small `prior_covariance` (the ridge
parameter) is used, together with a moderate leverage threshold — only the
top 90% leverage points are kept to fit the hypercube ensemble.
...
We compute mean predictions on the test set, along with min–max bounds and
...
Finally we check calibration by comparing true residuals to the predicted
uncertainties on the test set, broken down by property (energies, forces,
virials).

```julia
function data_masks(data_list, Ntot)  # boolean masks for E / F / V rows
...
To propagate these parameter uncertainties all the way to a physical
observable, continue to the
[molecular dynamics example](julia-md.md), which reuses this potential.
